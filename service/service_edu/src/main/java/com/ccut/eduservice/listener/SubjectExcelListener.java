package com.ccut.eduservice.listener;
...
public class SubjectExcelListener extends AnalysisEventListener<SubjectData> {
    /*因为subject excel listener不能交给spring管理，需要自己new，不能注入其他对象，不能实现数据库操作*/
...
//读取excel内容，一行一行进行读取
    ...
    if (subjectData ==null){
        throw new JiangDiException(20001,"文件数据为空");
    }
    //按行读取，每次读取两个值，第一个值一级分类，第二个值二级分类
        //判断一级分类是否重复
